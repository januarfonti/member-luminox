...
//    GET DATA
    ...
    function getAllPelanggan(){
        return $this->db->query("select * from user_profiles ")->result();
    }
...
//    INSERT DATA
    ...
    function insertPelanggan($data){
        $query = $this->db->insert('user_profiles',$data);
        return $query;
    }
...
//    UPDATE DATA
    ...
    function updatePelanggan($id,$data){
        $this->db->where('id',$id);
        $update = $this->db->update('user_profiles',$data);
        return $update;
    }
...
//    DELETE DATA
    ...
    function deletePelanggan($id){
        $this->db->where('id',$id);
        $delete = $this->db->delete('user_profiles');
        return $delete;
    }
...
//    KODE BARANG
    ...
    {
        $q = $this->db->query("select MAX(RIGHT(id,6)) as kd_max from user_profiles");
        $kd = "";
        if($q->num_rows()>0)
        {
            foreach($q->result() as $k)
