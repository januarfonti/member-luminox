...
class Auth extends CI_Controller
{
	...
	function index()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			if($this->tank_auth->permit('akses admin')){
							redirect('admin');
						}
						else if($this->tank_auth->permit('akses user')){
							redirect('member');
						}
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
					...
					if($this->tank_auth->is_approved()){
						...
						else if($this->tank_auth->permit('akses admin 2')){
							redirect('admin');
						}
						else if($this->tank_auth->permit('akses admin cp')){
							redirect('admin');
						}
						else if($this->tank_auth->permit('akses admin pp')){
							redirect('admin');
						}
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->change_password(
						...
						$this->session->set_flashdata('ganti_password', '<div class="alert alert-success alert-block fade in">
    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
    <strong>Password anda berhasil diganti.</strong>
  </div>');
			...
			//$this->load->view('auth/change_password_form', $data,$dt);
		}
	}
